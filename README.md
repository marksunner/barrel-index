...
Better still, an observed entry can be *measured* rather than estimated. Place a `barrel-ledger.json` file beside the page (see `barrel-ledger.example.json`) and record each real top-up in it: the date, what you paid in your own currency, and how many barrels (million tokens) that pool consumed since the previous top-up, which your gateway logs or provider console can tell you. The board then computes your true effective rate as money in divided by barrels through, averaged over your most recent top-ups (five by default) so it tracks drift rather than history, and labels the row `measured` with the sample count, the date of the last sample, and the implied multiple of the current list price. If the ledger file is missing or empty the row falls back gracefully to the multiplier estimate. There is also a zero-file route: every observed row carries a **Calibrate** button that opens a small panel directly on the page, where you enter what you paid, in which currency, and how many barrels it bought; the board recalibrates instantly, labels the row `calibrated` with the sample count, and remembers your samples in that browser. Precedence is deliberate and simple: page samples override the ledger file, which overrides the estimate, and each row states which source it is currently using. Page samples live in one browser's storage, so for a record that survives machines and browsers, or that other software can append to, prefer the ledger file. Because the rate is derived from real spending it already blends input and output tokens, so it is deliberately unaffected by the blend control. Measured mode requires the page to be served over HTTP (any static server, including an internal one); opened directly from disk it falls back to the estimate. The ledger is also a natural target for automation: anything that can append a line of JSON after each top-up keeps the board honest with no manual effort at all.
